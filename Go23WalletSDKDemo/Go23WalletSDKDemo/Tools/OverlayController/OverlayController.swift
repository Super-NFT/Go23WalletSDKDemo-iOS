...
private extension UIView {
    ...
    private func remove(overlay which: DissmissOptions,
        ...
        switch which {
        case .first:
            if let firstVc = controllersSet.first {
                dissmiss(overlay: firstVc, duration: duration, options: options, completion: completion)
            }
        case .last:
            if let lastVc = controllersSet.last {
                dissmiss(overlay: lastVc, duration: duration, options: options, completion: completion)
            }
        default:
